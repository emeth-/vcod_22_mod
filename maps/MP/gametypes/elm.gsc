...
main()
{
	...
	if(getCvar("scr_elm_timelimit") == "")		// Time limit per map
		setCvar("scr_elm_timelimit", "30");
	else if(getCvarFloat("scr_elm_timelimit") > 1440)
		setCvar("scr_elm_timelimit", "1440");
	level.timelimit = getCvarFloat("scr_elm_timelimit");
	setCvar("ui_elm_timelimit", level.timelimit);
	makeCvarServerInfo("ui_elm_timelimit", "30");

	if(getCvar("scr_elm_scorelimit") == "")		// Score limit per map
		setCvar("scr_elm_scorelimit", "1000");
	level.scorelimit = getCvarInt("scr_elm_scorelimit");
	setCvar("ui_elm_scorelimit", level.scorelimit);
	makeCvarServerInfo("ui_elm_scorelimit", "1000");
...
{
	level.elm_requires_speed_reset = false;
	...
	maps\mp\gametypes\_elm::ELMSetup();
...
Callback_PlayerConnect()
{
	...
	self iprintlnbold(" ^7You have 10 lives. Kill others, don't die.");	
	self iprintlnbold(" ^7You gain a life for every 5 kills, or taking someone out of the game.");

	iprintln(" ^7You have 10 lives. Kill others, don't die.");	
	iprintln(" ^7You gain a life for every 5 kills, or taking someone out of the game.");
	...
	players = getentarray("player", "classname");
			for(i = 0; i < players.size; i++)
			{
				...
				if(players[i].deaths>0)
				{
					...
					totlives = totlives + players[i].deaths;
				}
			}
	if(playerson<=0)
	{
	...
	{
		self.deaths = totlives / playerson;
	}
	...
	if(self.sessionstate != "dead")
	{
		if(sMeansOfDeath == "MOD_FALLING" && self.maxspeed > 190) // fall damage
		{
			...
			if(speedloss > 0)
			{
				...
				self iprintln(&"ELM_FALL_DAMAGE", speedloss);
			}
		}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc)
{
	...
	self.deaths--;
	attackerbonus = 0;
		self iprintlnbold(self.name + " ^7has "+self.deaths+" ^7lives left!");
	if(self.deaths <=0)
	{
				iprintln(self.name + " ^7has lost their last ^1life^7.");
				self iprintlnbold(" ^7You have lost your last ^1life^7!");
					//force player to spectate after no more lifes
					self.pers["team"] = "spectator";
	...
	if(isPlayer(attacker))
	{
		...
		else
		{
			...
			attacker.score += 1;
			if(attacker.score % 5 == 0 && attacker.score != 0)
			{
				//free life!
				attacker.deaths = attacker.deaths + 1;
				iprintln(attacker.name + " ^7just got a free life for killing 5 people!");	
				self iprintlnbold(attacker.name + " ^7just got a free life for killing 5 people!");
			}

			if(attackerbonus == 1)
			{
				//took someone out of game
				attacker.deaths = attacker.deaths + 1;
				iprintln(attacker.name + " ^7just got a free life for wiping "+noobname+"^7 out of the game!");	
				self iprintlnbold(attacker.name + " ^7just got a free life for wiping "+noobname+"^7 out of the game!");
			}
...
if(self.deaths>0)
{
	...
	self setClientCvar("cg_objectiveText", &"ELM_OBJECTIVE");

	self thread maps\mp\gametypes\_elm::ELMManageHUD();
}
}
...
respawn()
{
	//Currently, if they ran out of lives they just can't respawn. Should fix, force them to spectator or something...
	if(self.deaths>0)
	{
		if(!isDefined(self.pers["weapon"]))
			return;
...
checkScores()
{
	...
	if(playersleft  > 1 || (totplayers == 1 && playersleft != 0))
		return;
	if(level.mapended)
		return;
	level.mapended = true;
...
updateGametypeCvars()
{
	for(;;)
	{
		timelimit = getCvarFloat("scr_elm_timelimit");
		...
		scorelimit = getCvarInt("scr_elm_scorelimit");
