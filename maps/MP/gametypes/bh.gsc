...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc)
{
	...
	{
		if(isDefined(attacker.BHTarget))
		{
			if(attacker.BHTarget == self || attacker.BHHitman == self) // killed target
			{
		...
		attacker maps\mp\gametypes\_bh::BHAssignTarget();
		attacker iprintlnbold("^7Your Target (attacker): " + self.BHTarget.name);
...
//	}
	...
	self setClientCvar("cg_objectiveText", &"BH_OBJECTIVE");
}
...
respawn()
{
	...
	self maps\mp\gametypes\_bh::BHAssignTarget();
	self iprintlnbold("^7Your Target(you are spawning): " + self.BHTarget.name);
}
