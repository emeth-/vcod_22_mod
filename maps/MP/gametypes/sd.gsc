...
/////////////////////////////////////////////////
	...
	if(!isDefined(game["gamestarted"]))
	{
		...
		precacheString(&"SD_OBJ_ATTACKERS");
		precacheString(&"SD_OBJ_DEFENDERS");
	    ...
	    precacheItem("sbknife_mp");
    	precachemodel("xmodel/weapon_kar98k_sniper_mp");
    	precachemodel("xmodel/barrel_benzin_stalingrad");
    	precacheString(&"SD_HEALTH_TXT");
    	precacheString(&"SD_BOOMER_INFO");
    	precacheString(&"SD_BOOMER_INFO2");
    	precacheString(&"SD_TANK_INFO");
    	precacheString(&"SD_TANK_INFO2");
    	precacheString(&"SD_POISON_INFO");
    	precacheString(&"SD_POISON_INFO2");
    	precacheString(&"SD_SPRINT_INFO");
    	precacheString(&"SD_SPRINT_INFO2");
    	precacheString(&"SD_SPRINT_INFO3");
    	precacheString(&"SD_TELEPORT_INFO");
    	precacheString(&"SD_TELEPORT_INFO2");
    	precacheString(&"SD_LAZY_SPRINT_INFO");
    	precacheString(&"SD_LAZY_SPRINT_INFO2");
    	precacheString(&"SD_LEAPER_INFO");
    	precacheString(&"SD_LEAPER_INFO2");
    	precacheString(&"SD_LEAPER_INFO3");
    	precacheString(&"SD_LEAPER_INFO4");
    	precacheString(&"SD_LEAPER_INFO5");
    	precacheString(&"SD_HUMAN_INFO");
    	precacheString(&"SD_HUMAN_INFO2");
    	precacheString(&"SD_HUMAN_INFO3");
    	precacheString(&"SD_HUMAN_INFO4");
    	precacheString(&"SD_HUMAN_INFO5");
...
Callback_PlayerConnect()
{
	...
	self thread resetGammaConstantly();
	for(;;)
	{
		...
		else if(menu == game["menu_weapon_allies"] || menu == game["menu_weapon_axis"])
		{
			...
			if(!game["matchstarted"])
			{
				if(isDefined(self.pers["weapon"]))
				{
                	//game["zombieteam"] == 1 means allies=human, axis=zombie
                	//game["zombieteam"] == 0 means allies=zombie, axis=human
                	if((self.pers["team"] == "axis" && game["zombieteam"] == 0) || (self.pers["team"] == "allies" && game["zombieteam"] == 1))
                	{
...
resetGammaConstantly()
{
	for(;;)
	{
		self setClientCvar("r_gamma", "1");
		wait 3;
	}
}
...
///////////////////////////
	...
	if(sMeansOfDeath == "MOD_FALLING" && self.zombietype == 3) // fall damage, leaper zombie
	{

           range = 275;
           maxdamage = iDamage;
           if(maxdamage>90){maxdamage=90;}
           mindamage = maxdamage/4;

           radiusDamage(self.origin + (0,0,12), range, maxdamage, mindamage);
	}
	
	if(sMeansOfDeath == "MOD_GRENADE_SPLASH" && self.zombietype > 0)
	{
		self shellshock("default", 3);
	}
	if(!isPlayer(attacker) && sMeansOfDeath == "MOD_EXPLOSIVE" && self.zombietype <= 0)
	{
		self shellshock("default", 3);
	}
	if(isPlayer(eAttacker) && sMeansOfDeath == "MOD_MELEE" && eAttacker.zombietype ==1)
	{
		self shellshock("default", 1);
	}
	if(isPlayer(eAttacker) && sMeansOfDeath == "MOD_MELEE" && eAttacker.zombietype == 0 && self.zombietype > 0)
	{
		self shellshock("default", 3);
	}
	
	if(sMeansOfDeath == "MOD_FALLING" && self.zombietype>0)
	{
		skipdamage=1;
	}
	if(sMeansOfDeath == "MOD_EXPLOSIVE" && self.zombietype==3)
	{
		skipdamage=1;
	}

	// check for completely getting out of the damage
	if(!(iDFlags & level.iDFLAGS_NO_PROTECTION) && skipdamage==0)
	{
...
///////////////////////////////////////////////
		...
		else
		{
			...
			if(sMeansOfDeath == "MOD_RIFLE_BULLET" && eAttacker.zombietype == 6) // teleport zombie, swap places
			{
				iDamage = 1;
				tmp = self.Origin;
				
				self setorigin((eAttacker.Origin));
				
				eAttacker setorigin((tmp));
				
				self iprintln ("You just got nailed by a teleport zombie.");
			}
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc)
{
	...
	storeoriginofdeath = self.origin;
	if(self.sessionteam == "spectator")
		return;
...
////////////////////////////////////
	...
	if(self.zombietype != 4 || self.exploded==1)
	{
		// send out an obituary message to all clients about the kill
	    obituary(self, attacker, sWeapon, sMeansOfDeath);
	}
	if (isdefined (self.zombiehud))
		self.zombiehud destroy();
	if (isdefined (self.zombiehud2))
		self.zombiehud2 destroy();
	if (isdefined (self.zombiehud3))
		self.zombiehud3 destroy();
	if (isdefined (self.zombiehud4))
		self.zombiehud4 destroy();
	if (isdefined (self.zombiehud5))
		self.zombiehud5 destroy();
		
	if (isdefined (self.humanhud1))
		self.humanhud1 destroy();
	if (isdefined (self.humanhud2))
		self.humanhud2 destroy();
	if (isdefined (self.humanhud3))
		self.humanhud3 destroy();
	if (isdefined (self.humanhud4))
		self.humanhud4 destroy();
	if (isdefined (self.humanhud5))
		self.humanhud5 destroy();
		
	if (isdefined (self.healthtxthud))
		self.healthtxthud destroy();
	if (isdefined (self.healthnumhud))
		self.healthnumhud destroy();
	...
	{
		if(sMeansOfDeath == "MOD_MELEE" && attacker.zombietype > 0 && self.zombietype <= 0) 
		{
			attacker.score++;
		}
		if(self.zombietype > 0 && attacker.zombietype <= 0)
		{
			attacker giveMaxAmmo(attacker.pers["weapon"]);
			//attacker.tmphealth = attack.health + 15; 
			
			attacker.newhealthamt=(attacker.health+20);
			if (attacker.newhealthamt>attacker.maxhealth)
			{
				attacker.newhealthamt=attacker.maxhealth;
			}
			attacker.health = attacker.newhealthamt;
		...
		if(attacker == self) // killed himself
		{
			...
			if (!isdefined (self.autobalance) && self.zombietype <= 0)
			{
				attacker.pers["score"]--;
				attacker.score = attacker.pers["score"];
			}
			
			if(self.zombietype <= 0 && self.exploded==0)
			{
...
///////////////////////////////////////////////
	...
	if(!isdefined (self.autobalance) && (self.zombietype != 4 || self.exploded==1))
	{
	...
	if(self.zombietype == 4 && self.exploded!=1)
	{
       ...
       maxdamage = 190;
       mindamage = 30;
       playfx(level._effect["bombexplosion"], storeoriginofdeath);
       radiusDamage(storeoriginofdeath + (0,0,12), range, maxdamage, mindamage);
		wait 0.05;
	}
...
spawnPlayer()
{
	...
	//game["zombieteam"] == 1 means allies=human, axis=zombie
	//game["zombieteam"] == 0 means allies=zombie, axis=human
	if((self.pers["team"] == "axis" && game["zombieteam"] == 1) || (self.pers["team"] == "allies" && game["zombieteam"] == 0))
	{
		self.zombietype = randomInt(7); 
		self.zombietype = self.zombietype + 1; 
		if(self.zombietype==1||self.zombietype==2)
		{
			self.zombietype = 2; //speed
		}
		if(self.zombietype==3||self.zombietype==4)
		{
			self.zombietype = 4; //boomer
		}
		if(self.zombietype==5)
		{
			self.zombietype = 3; //leaper
		}
		if(self.zombietype==6)
		{
			self.zombietype = 1; //tank
		}
		if(self.zombietype==7)
		{
			self.zombietype = 6; //teleport
		}
		wait 0.01;
	}
	else
	...
	if((self.pers["team"] == "axis" && game["zombieteam"] == 1) || (self.pers["team"] == "allies" && game["zombieteam"] == 0))
	{
		...
		wait 0.05;
		self switchtoweapon("sbknife_mp");
	}
	...
	if((self.pers["team"] == "axis" && game["zombieteam"] == 0) || (self.pers["team"] == "allies" && game["zombieteam"] == 1))
	{
			if (!isdefined (self.humanhud1))
			{
				self.humanhud1 = newClientHudElem(self);
				self.humanhud1.alignX = "center";
				self.humanhud1.alignY = "middle";
				self.humanhud1.x = 540;
				self.humanhud1.y = 150;
				self.humanhud1.fontScale = 1;
				self.humanhud1 setText(&"SD_HUMAN_INFO");
			}
			if (!isdefined (self.humanhud2))
			{
				self.humanhud2 = newClientHudElem(self);
				self.humanhud2.alignX = "center";
				self.humanhud2.alignY = "middle";
				self.humanhud2.x = 540;
				self.humanhud2.y = 160;
				self.humanhud2.fontScale = 0.6;
				self.humanhud2 setText(&"SD_HUMAN_INFO2");
			}
			if (!isdefined (self.humanhud3))
			{
				self.humanhud3 = newClientHudElem(self);
				self.humanhud3.alignX = "center";
				self.humanhud3.alignY = "middle";
				self.humanhud3.x = 540;
				self.humanhud3.y = 168;
				self.humanhud3.fontScale = 0.6;
				self.humanhud3 setText(&"SD_HUMAN_INFO3");
			}
			if (!isdefined (self.humanhud4))
			{
				self.humanhud4 = newClientHudElem(self);
				self.humanhud4.alignX = "center";
				self.humanhud4.alignY = "middle";
				self.humanhud4.x = 540;
				self.humanhud4.y = 176;
				self.humanhud4.fontScale = 0.6;
				self.humanhud4 setText(&"SD_HUMAN_INFO4");
			}	
			if (!isdefined (self.humanhud5))
			{
				self.humanhud5 = newClientHudElem(self);
				self.humanhud5.alignX = "center";
				self.humanhud5.alignY = "middle";
				self.humanhud5.x = 540;
				self.humanhud5.y = 184;
				self.humanhud5.fontScale = 0.6;
				self.humanhud5 setText(&"SD_HUMAN_INFO5");
			}	
	}
	
	
	while(isalive(self) && (self.pers["team"] == "axis" && game["zombieteam"] == 0) || (self.pers["team"] == "allies" && game["zombieteam"] == 1))
	{
		if(self.health <= 25)
		{
	...
	if((self.pers["team"] == "axis" && game["zombieteam"] == 1) || (self.pers["team"] == "allies" && game["zombieteam"] == 0))
	{
		...
		{
			self iprintlnbold ("Your are a Tank Zombie.");
			self.health = 1500;
			self.maxspeed = 210;
			...
			if (!isdefined (self.zombiehud))
			{
				...
				self.zombiehud.alignX = "center";
				self.zombiehud.alignY = "middle";
				self.zombiehud.x = 540;
				self.zombiehud.y = 150;
				self.zombiehud.fontScale = 1;
				//self.zombiehud.archived = false;
				//self.zombiehud.label = (&"HQ_REINFORCEMENTS");
				//self.zombiehud setTimer (level.counter);
				self.zombiehud setText(&"SD_TANK_INFO");
			}
			if (!isdefined (self.zombiehud2))
			{
				self.zombiehud2 = newClientHudElem(self);
				self.zombiehud2.alignX = "center";
				self.zombiehud2.alignY = "middle";
				self.zombiehud2.x = 540;
				self.zombiehud2.y = 160;
				self.zombiehud2.fontScale = 0.6;
				self.zombiehud2 setText(&"SD_TANK_INFO2");
			}
		}
		
		if(self.zombietype==2)
		{
			self iprintlnbold ("Your are a Sprinting Zombie.");
			self.health = 250;
			if (!isdefined (self.zombiehud))
			{
				self.zombiehud = newClientHudElem(self);
				...
				//self.zombiehud.label = (&"HQ_REINFORCEMENTS");
				//self.zombiehud setTimer (level.counter);
				self.zombiehud setText(&"SD_SPRINT_INFO");
			}
			if (!isdefined (self.zombiehud2))
			{
				self.zombiehud2 = newClientHudElem(self);
				...
				self.zombiehud2.y = 160;
				self.zombiehud2.fontScale = 0.6;
				self.zombiehud2 setText(&"SD_SPRINT_INFO2");
			}
			if (!isdefined (self.zombiehud3))
			{
				self.zombiehud3 = newClientHudElem(self);
				self.zombiehud3.alignX = "center";
				self.zombiehud3.alignY = "middle";
				self.zombiehud3.x = 540;
				self.zombiehud3.y = 168;
				self.zombiehud3.fontScale = 0.6;
				self.zombiehud3 setText(&"SD_SPRINT_INFO3");
			}
		}
		if(self.zombietype==3)
		{
			self iprintlnbold ("Your are a Leaping Zombie.");
			self.health = 250;
			self.directionx = 0;
			self.directiony = 0;
			self.directionz = 0;
			if (!isdefined (self.zombiehud))
			{
				...
				//self.zombiehud.label = (&"HQ_REINFORCEMENTS");
				//self.zombiehud setTimer (level.counter);
				self.zombiehud setText(&"SD_LEAPER_INFO");
			}
			if (!isdefined (self.zombiehud2))
			{
				self.zombiehud2 = newClientHudElem(self);
				...
				self.zombiehud2.y = 160;
				self.zombiehud2.fontScale = 0.6;
				self.zombiehud2 setText(&"SD_LEAPER_INFO2");
			}
			if (!isdefined (self.zombiehud3))
			{
				self.zombiehud3 = newClientHudElem(self);
				...
				self.zombiehud3.y = 168;
				self.zombiehud3.fontScale = 0.6;
				self.zombiehud3 setText(&"SD_LEAPER_INFO3");
			}
			if (!isdefined (self.zombiehud4))
			{
				self.zombiehud4 = newClientHudElem(self);
				self.zombiehud4.alignX = "center";
				self.zombiehud4.alignY = "middle";
				self.zombiehud4.x = 540;
				self.zombiehud4.y = 176;
				self.zombiehud4.fontScale = 0.6;
				self.zombiehud4 setText(&"SD_LEAPER_INFO4");
			}
			if (!isdefined (self.zombiehud5))
			{
				self.zombiehud5 = newClientHudElem(self);
				self.zombiehud5.alignX = "center";
				self.zombiehud5.alignY = "middle";
				self.zombiehud5.x = 540;
				self.zombiehud5.y = 184;
				self.zombiehud5.fontScale = 0.6;
				self.zombiehud5 setText(&"SD_LEAPER_INFO5");
			}
		}
		
		if(self.zombietype==4)
		{
			self iprintlnbold ("Your are a Boomer Zombie.");
			self.health = 100;
			if (!isdefined (self.zombiehud))
			{
				self.zombiehud = newClientHudElem(self);
				...
				//self.zombiehud.label = (&"HQ_REINFORCEMENTS");
				//self.zombiehud setTimer (level.counter);
				self.zombiehud setText(&"SD_BOOMER_INFO");
			}
			if (!isdefined (self.zombiehud2))
			{
				self.zombiehud2 = newClientHudElem(self);
				...
				self.zombiehud2.y = 160;
				self.zombiehud2.fontScale = 0.6;
				self.zombiehud2 setText(&"SD_BOOMER_INFO2");
			}
		...
		if(self.zombietype==6)
		{
			self iprintlnbold ("Your are a Teleport Zombie.");
			self.health = 100;
			if (!isdefined (self.zombiehud))
			{
				self.zombiehud = newClientHudElem(self);
				...
				//self.zombiehud.label = (&"HQ_REINFORCEMENTS");
				//self.zombiehud setTimer (level.counter);
				self.zombiehud setText(&"SD_TELEPORT_INFO");
			}
			if (!isdefined (self.zombiehud2))
			{
				self.zombiehud2 = newClientHudElem(self);
				...
				self.zombiehud2.y = 160;
				self.zombiehud2.fontScale = 0.6;
				self.zombiehud2 setText(&"SD_TELEPORT_INFO2");
			}
		...
		if (!isdefined (self.healthtxthud))
		{
			self.healthtxthud = newClientHudElem(self);
			self.healthtxthud.alignX = "center";
			self.healthtxthud.alignY = "middle";
			self.healthtxthud.x = 540;
			self.healthtxthud.y = 196;
			self.healthtxthud.fontScale = 0.6;
			self.healthtxthud setText(&"SD_HEALTH_TXT");
		}
		if (!isdefined (self.healthnumhud))
		{
			self.healthnumhud = newClientHudElem(self);
			self.healthnumhud.alignX = "center";
			self.healthnumhud.alignY = "middle";
			self.healthnumhud.x = 560;
			self.healthnumhud.y = 196;
			self.healthnumhud.fontScale = 0.6;
			self.healthnumhud setValue((int)(self.health));
		}

		
		while(isalive(self) && (self.zombietype == 3 || self.zombietype == 2))
		{
			wait 0.05;
			if (self useButtonPressed())
				self.useheld = self.useheld + 0.05;
			else
				self.useheld = 0;
				
			if (self meleeButtonPressed())
				self.meleeheld = self.meleeheld + 0.05;
			else
				self.meleeheld = 0;
				
			if(self.zombietype == 3 && self attackButtonPressed())
			{
			
				temp_vec = self.origin;	
				self.directionnewx = temp_vec[0];
				self.directionnewy = temp_vec[1];
				diffx = self.directionnewx - self.directionx;
				diffy = self.directionnewy - self.directiony;
				diffx = diffx * 3;
				diffy = diffy * 3;
				if(diffy > 700){diffy = 700;}
				if(diffy < -700){diffy = -700;}
				if(diffx > 700){diffx = 700;}
				if(diffx < -700){diffx = -700;}
	
				//self.directionnewz = temp_vec[2];
				throw = (diffx, diffy, 900); //direction of launch 
				dest = spawn ("script_model",(0,0,0)); //spawn a script_model
				dest.origin = self.origin; //give script_model same origin as user
				dest.angles = self.angles; //give script_model same angle as user
				self linkto (dest); //link the user to the script_model
				dest movegravity (throw, 15); //throw the script model (and the player)
				wait 0.5; //wait
				self unlink(); //unlink the player from the script_model
				wait 0.5; //wait 
			}
			
				
			if(self.zombietype == 2 && self attackButtonPressed())
			{
				if(self.maxspeed == 350)
				{
					self.maxspeed = 190;
					wait 2;
				}
			...
			if (self.useheld == 0.3 && self.zombietype==3)
			{
				temp_vec = self.origin;	
				self.directionx = temp_vec[0];
				self.directiony = temp_vec[1];
				self.directionz = temp_vec[2];
				self iprintlnbold ("Running direction marked. Press FIRE to leap.");
			}		
			if (self.meleeheld == 0.5 && self.zombietype==3)
			{
            	if(self.pers["team"] == "allies")
            		spawnpointname = "mp_searchanddestroy_spawn_allied";
            	else
