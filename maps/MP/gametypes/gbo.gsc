main()
{
	...
	if(getCvar("scr_gbo_timelimit") == "")		// Time limit per map
		setCvar("scr_gbo_timelimit", "30");
	else if(getCvarFloat("scr_gbo_timelimit") > 1440)
		setCvar("scr_gbo_timelimit", "1440");
	level.timelimit = getCvarFloat("scr_gbo_timelimit");
	setCvar("ui_gbo_timelimit", level.timelimit);
	makeCvarServerInfo("ui_gbo_timelimit", "30");

	if(getCvar("scr_gbo_scorelimit") == "")		// Score limit per map
		setCvar("scr_gbo_scorelimit", "1000");
	level.scorelimit = getCvarInt("scr_gbo_scorelimit");
	setCvar("ui_gbo_scorelimit", level.scorelimit);
	makeCvarServerInfo("ui_gbo_scorelimit", "1000");
...
{
	level.gbo_requires_speed_reset = false;
	...
	maps\mp\gametypes\_gbo::GBOSetup();
	...
	if(self.sessionstate != "dead")
	{
		if(sMeansOfDeath == "MOD_FALLING" && self.maxspeed > 190) // fall damage
		{
			...
			if(speedloss > 0)
			{
				...
				self iprintln(&"GBO_FALL_DAMAGE", speedloss);
			}
		}
...
spawnPlayer()
{
	...
	self setClientCvar("cg_objectiveText", &"GBO_OBJECTIVE");
}
...
updateGametypeCvars()
{
	for(;;)
	{
		timelimit = getCvarFloat("scr_gbo_timelimit");
		...
		scorelimit = getCvarInt("scr_gbo_scorelimit");
